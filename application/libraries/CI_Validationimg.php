...
class CI_Validationimg
{
	private $width,$height,$codenum;
	public $checkcode;     //产生的验证码
	private $checkimage;    //验证码图片
	private $disturbColor = ''; //干扰像素
	function check($width='80',$height='20')
	{
		if(isset($_GET['auth_img'])&&md5($width.'def'.$height)==$_GET['auth_img'])
			return true;
		else
			return false;
	}

	function get_auth_img($width='80',$height='20')
	{
		return md5($width.'def'.$height);
	}
	
	function validationvcode($width='80',$height='20',$codenum='4')
	{
	   $this->width=$width;
	   $this->height=$height;
